...
`max-func-body-length` | Function body length must not exceed value configured within this rule options. You can setup a general max function body length applied for every function/method/arrow function e.g. \[true, 30\] or set different maximum length for every type e.g. \[true, \["func-body-length", 10\], \["arrow-body-length", 5\], \[ "method-body-length", 15 \]\] | 1.0
