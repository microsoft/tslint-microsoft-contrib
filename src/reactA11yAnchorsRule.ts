...
const LINK_CHAR: string = ' First link at character: '; // Do we have an alternative to concatenating strings...
const LINK_LINE: string = ' line: ';
...
class ReactA11yAnchorsRuleWalker extends ErrorTolerantWalker {
    ...
    public validateAllAnchors(): void {
        ...
        for (let i = 0; i < this.anchorInfoList.length; i++) {
            const current: AnchorInfo = this.anchorInfoList[i];

            for (let j = i; j < this.anchorInfoList.length; j++) {
                const anchorInfo: AnchorInfo = this.anchorInfoList[j];

                if (current.href === anchorInfo.href &&
                    current.text !== anchorInfo.text &&
                    !_.include(sameHrefDifferentTexts, anchorInfo)) {
                ...
                if (current.href !== anchorInfo.href &&
                    current.text === anchorInfo.text &&
                    !_.include(differentHrefSameText, anchorInfo)) {
    ...
    /* tslint:enable:underscore-consistent-invocation */
    ...
    private firstPosition(anchorInfo: AnchorInfo): string {
        ...
        return LINK_CHAR + character + LINK_LINE + line;
    }
