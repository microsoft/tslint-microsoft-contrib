...
class ReactA11yTitlesRuleWalker extends Lint.RuleWalker {
    ...
    private validateTitleText(text: string, titleNode: ts.Node): void {
        if (text.length > MAX_TITLE_LENGTH) {
            this.addFailureAt(titleNode.getStart(), titleNode.getWidth(), LONG_TITLE_FAILURE_STRING + ': ' + Utils.trimTo(text, 20));
        } else if (!(text.indexOf(' ') > 0)) {
            this.addFailureAt(titleNode.getStart(), titleNode.getWidth(), WORD_TITLE_FAILURE_STRING + ': ' + Utils.trimTo(text, 20));
        }
    }
}
